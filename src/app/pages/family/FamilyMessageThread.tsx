...
import { ChevronLeft, Send, Mic, Phone, Video, Image as ImageIcon, X } from "lucide-react";
...
// Thread-specific message data (inverted roles for family view)
const THREAD_MESSAGES: Record<string, Message[]> = {
  grandma: [
    ...
    { sender: 'me', text: '', type: 'voice', duration: 12, time: '8:24 AM', id: 1 },
    ...
    { sender: 'them', text: '', type: 'voice', duration: 18, time: '8:36 AM', id: 2 },
...
export function FamilyMessageThread() {
  ...
  const gradient = avatarGradients[initial] || avatarGradients['G'];
  ...
  // Grandma thread — scroll to top on mount
  useLayoutEffect(() => {
    if (contactId === 'grandma' && scrollContainerRef.current) {
      scrollContainerRef.current.scrollTop = 0;
    }
  }, [contactId]);

  // Auto scroll to bottom (only for non-Grandma threads)
  useEffect(() => {
    // Skip for Grandma thread
    if (contactId === 'grandma') return;
    ...
    >
      ...
      >
        <div className="flex items-center gap-4">
          <button 
            onClick={() => { playSound("pageBackChime"); navigate("/family/messages"); }}
            style={{ color: '#C4601A', cursor: 'pointer' }}
          >
            <ChevronLeft size={20} />
          </button>
      ...
      {/* Messages */}
      <div className="flex-1 px-6 py-6 overflow-y-auto relative z-10" ref={scrollContainerRef}>
        {messages.length === 0 && (
          <div 
            style={{
              position: 'fixed',
      ...
      {/* Global cursor styles + Animation keyframes */}
      <style>{`
